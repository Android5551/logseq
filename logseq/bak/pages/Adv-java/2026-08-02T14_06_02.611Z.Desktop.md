	  ...
	  collapsed:: true
		...
		- ### Code:
			...
			- you can call add() using creating that class object and call using that obj function
				...
				-
- [[Fri, 31.07.2026]]
	- _To avoid creating 16 columns in add method use:_
		- Create `bean` class with private attributes
			- Add `getters` and `setters` for `id` `firstName` etc.
			- `dob` will be of `java.util` type
		- and in add method remove all column names and add its object
		- how can i remove all column names and add the object of a bean class having attributes 
		  Remove column names by omitting @Column annotations.  
		  Use bean properties as column names by default.  
		  Map database columns to bean attributes via annotations.  
		  Ensure bean attributes match database column names.  
		  Use @Column(name = "") to explicitly map properties to columns.  
		  Avoid specifying column names if using auto-mapping.  
		  Configure ORM to use property names as column names.  
		  Add bean object to database operations for automatic mapping.  
		  Verify property names match column names or use annotations for mapping.
		-
		-
-